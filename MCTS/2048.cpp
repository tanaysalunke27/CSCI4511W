...
void play_game(get_move_func_t get_move) {
	for(int i=1; i<11; i++) {
		...
		bool reached_2048 = false; // Flag to track if the 2048 tile is reached
		int moveno_2048 = 0;       // Move number when the 2048 tile is reached
		time_t time_2048 = 0; // Time when 2048 is reached
		int move_number = 0;
		...
		while (1) {
			...
			// Insert a new tile and update the board
			board_t tile = draw_tile();
			if (tile == 2) scorepenalty += 4;
			board = insert_tile_rand(newboard, tile);

			// Check if 2048 tile has been reached
			if (!reached_2048 && get_max_rank(board) >= 11) { // Rank 11 corresponds to 2^11 = 2048
                reached_2048 = true;
                time_2048 = time(NULL); // Record the time
                moveno_2048 = move_number; // Record the move number
                double elapsed_time = difftime(time_2048, start_time);
                printf("2048 tile was achieved at move #%d, time: %f \n", moveno_2048, elapsed_time);
                //printf("\n2048 tile achieved at move #%d, time: %s", moveno, ctime(&time_2048));
			}
		}

		time_t end_time = time(NULL);
        print_board(board);
		int highest_tile = pow(2,get_max_rank(board));
        printf("\nGame over. Your score is %.0f. The highest tile you achieved was %d.\n",
            score_board(board) - scorepenalty, highest_tile);
        double total_time = difftime(end_time, start_time);
        printf("Total time: %f\n", total_time);
        //printf("2048 tile was achieved at move #%d, time: %s", move_2048, elapsed_time);
        printf("-----------------------------------------------------------");
        printf("\n");
        printf("\n");
		// Print the move number when 2048 was reached, if applicable
	}
}
...
//			if (score < 0) console.log('bugggg');
			...
			if ((LEVELUP_COUNT > 0) && (get_max_rank(board)>10) && (deepSearch==0)) {
				...
				i = 0;
				bestScore = 0;
				//Change the rollout number here
				//RUNS = 20;
				//RUNS = 50;
				//RUNS = 100;
				//RUNS = 1000;
