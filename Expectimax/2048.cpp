...
float score_toplevel_move(board_t board, int move) {
    ...
    double elapsed;
    eval_state state;
    //state.depth_limit = std::max(3, count_distinct_tiles(board) - 2); // dynamic depth adjustment
    //state.depth_limit = 2; // fixed depth of 2
    //state.depth_limit = 4; // fixed depth of 4
    state.depth_limit = 8; // fixed depth of 8
...
void play_game(get_move_func_t get_move) {
    for(int i=1; i<11; i++) {
        ...
        bool reached_2048 = false; // Flag to track if 2048 has been reached
        time_t time_2048 = 0; // Time when 2048 is reached
        int move_2048 = 0; // Move number when 2048 is reached
        int move_number = 0;
